...
def sub_trajectory_to_sequence(s_t,trajectory_num):
    line = 'start,%d\n'%(trajectory_num)
    for x in s_t:
        line += '%s,%d\n'%('-'.join(map(str,x)),trajectory_num)
    line += 'end,%d\n'%(trajectory_num)
    return line 
...
trajectory_sequence_extract_path = '../data/trajectory_sequence'
...
trajectory_sequence_extract = open(trajectory_sequence_extract_path,'w')
...
for raw_data_file in raw_data_files:
    ...
    if len(time_list) > min_len:
        ...
        for s_t in sub_trajectory_part:
            ...
            trajectory_sequence_extract.write(sub_trajectory_to_sequence(s_t,trajectory_num))
            trajectory_num += 1
...
trajectory_sequence_extract.close()
